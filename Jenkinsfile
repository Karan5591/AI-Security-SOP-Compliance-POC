...
pipeline {
    ...
    parameters {
        // NEW: Add the target repo URL and branch
    ...
    stages {
        // NEW STAGE: Clone the target repository into a subfolder
...
PY
        ...
        stage('Build and test images') {
            steps {
                // Again, we cd into the target repo to build IT, not the RAG tool
                sh '''
                    set -eu
                    cd target-repo
                    docker build --tag "${API_IMAGE}" .
